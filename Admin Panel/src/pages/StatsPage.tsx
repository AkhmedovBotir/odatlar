...
export function StatsPage() {
  ...
  const loadLeaders = useCallback(async (lim: number) => {
    ...
    try {
      ...
      setError(err instanceof ApiClientError ? err.message : 'Reytingni yuklashda xato')
    } finally {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof ApiClientError ? err.message : 'Statistikani yuklashda xato')
    }
  }, [limit])
  ...
  return (
    <div>
      ...
      {stats && (
        <>
          <SectionTitle icon={Users} title="Foydalanuvchilar" />
          ...
          <SectionTitle icon={LineChart} title="Faoliyat" className="mt-8" />
          ...
          <SectionTitle icon={Zap} title="XP ko'rsatkichlari" className="mt-8" />
          <Card>
      ...
      <div className="mt-8 mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold text-slate-900">XP reytingi</h2>
          <span className="text-xs text-slate-400">Jami: {fmt(leadersTotal)}</span>
        </div>
        <div className="w-32">
          <Select
            value={limit}
