...
export function BotSettingsPage() {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      snackbar.error(getApiErrorMessage(err, 'Foydalanuvchilarni yuklashda xato'))
    } finally {
  ...
  async function handleSaveToken(e: FormEvent) {
    ...
    try {
      ...
      setNewToken('')
      snackbar.success('Bot token muvaffaqiyatli saqlandi')
    } catch (err) {
  ...
  async function handleDeleteToken() {
    ...
    try {
      ...
      await loadSettings()
      snackbar.success('Bot token o\'chirildi')
    } catch (err) {
  ...
  async function handleSaveStart(e: FormEvent) {
    ...
    if (start.button.enabled && (!start.button.text.trim() || !start.button.web_app_url.trim())) {
      snackbar.error('Tugma yoqilganda matn va Web App URL majburiy')
      return
    }
    ...
    try {
      ...
      setSettings(data)
      setStart(data.start)
      snackbar.success('/start sozlamalari saqlandi')
    } catch (err) {
      snackbar.error(getApiErrorMessage(err, 'Saqlashda xato'))
    } finally {
      setSaving(false)
  ...
  async function openUserModal(id: number) {
    ...
    try {
      ...
      snackbar.error(getApiErrorMessage(err, 'Foydalanuvchi ma\'lumotini olishda xato'))
    } finally {
