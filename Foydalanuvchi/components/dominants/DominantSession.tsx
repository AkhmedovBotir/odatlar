...
import {
  completeDominantSession,
  createDominant,
  hasTelegramSession,
} from '@/lib/dominantsApi';
...
export default function DominantSession({ dominantId }: DominantSessionProps) {
  ...
  const { userData, addReward, updateWithRewards, applyServerXPReward } = useUserData();
  ...
  const handleTimerComplete = useCallback(async () => {
    ...
    let xpAward: { xp_reward?: number } | null = null;

    if (dominantId) {
      if (hasTelegramSession()) {
        try {
          const updated = await completeDominantSession(dominantId, {
            type: tur,
            pros: prosList,
            cons: consList,
            notes,
          ...
          dominants = userData.dominants.map((d) =>
            d.id === dominantId ? updated : d
          );
          if (hasServerXPAward(updated)) xpAward = updated;
        } catch (error) {
          console.error('[DominantSession] session yakunlash xatosi', error);
          return;
        }
      } else {
        dominants = userData.dominants.map((d) =>
      ...
      if (hasTelegramSession()) {
        try {
          const created = await createDominant({
            title: draft.title.trim(),
            cue: draft.cue.trim(),
            reward: draft.reward.trim(),
            type: tur,
            pros: tur === 'fikrlash' ? prosList : [],
            cons: tur === 'fikrlash' ? consList : [],
            notes: tur === 'ma\'lumot' ? notes : '',
          });
          dominants = [...userData.dominants, created];
          if (hasServerXPAward(created)) xpAward = created;
        } catch (error) {
          console.error('[DominantSession] dominanta yaratish xatosi', error);
          return;
        }
      } else {
        dominants = [
    ...
    if (xpAward && hasServerXPAward(xpAward)) {
      applyServerXPReward(xpAward, { dominants });
      if (xpAward.xp_reward) addReward(xpAward.xp_reward, 'xp');
    } else {
      updateWithRewards({ dominants }, 100, 10);
      addReward(100, 'xp');
    }
    router.push('/dominantalar');
  }, [userData, tur, dominantId, prosList, consList, notes, addReward, updateWithRewards, applyServerXPReward, router]);
