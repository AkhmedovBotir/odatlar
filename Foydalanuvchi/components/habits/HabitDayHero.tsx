...
import { getLocalDateKey, isHabitActiveOnDate } from '@/lib/habits';
...
import type { HabitWeekDay } from '@/lib/habitSummaryApi';
...
function WeekStrip({
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const activeHabits = tracked.filter((habit) => isHabitActiveOnDate(habit, dateKey));
    ...
    const rate = getDayCompletionRate(logged, activeHabits.length);
      ...
      <div className="p-4 md:p-5">
        ...
        {tracked.length > 0 && (
          <div className="border-t border-slate-700/50 pt-3">
            ...
            {week && week.length > 0 ? (
              <WeekStripFromApi variant={variant} week={week} />
            ) : (
