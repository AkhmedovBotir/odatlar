...
export function getHabitCreatedDateKey(habit: GoodHabit): string | null {
  if (!habit.createdAt) return null;
  return habit.createdAt.slice(0, 10);
}

export function isHabitActiveOnDate(habit: GoodHabit, dateKey: string): boolean {
  const created = getHabitCreatedDateKey(habit);
  if (!created) return true;
  return dateKey >= created;
}

export function archiveDateFilterToRange(filter: ArchiveDateFilter): { from: string; to: string } {
  const normalized = normalizeArchiveDateFilter(filter);
  if (normalized.mode === 'day') {
    return { from: normalized.date, to: normalized.date };
  }
...
export function buildArchiveDays(
  ...
  return getArchiveFilterDateKeys(filter).flatMap((date) => {
    const activeHabits = filteredHabits.filter((habit) => isHabitActiveOnDate(habit, date));
    ...
    const missedItems: ArchiveDayItem[] = activeHabits
